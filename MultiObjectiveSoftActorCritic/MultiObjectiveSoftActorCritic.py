...
class SoftActorCritic:
    def __init__(self, name, environment, P_shape, Q_shape, replay_buffer_size, seed=None):
        ...
        self.__P = [np.random.normal(u, s, size=(environment.obs_sp_shape[-1]+environment.prf_sp_shape[-1]+1, P_shape[0])).astype(data_type), ]
        ...
        self.__Q = [np.random.normal(u, s, size=(2, environment.obs_sp_shape[-1]+environment.prf_sp_shape[-1]+environment.act_sp_shape[-1]+1, Q_shape[0])).astype(data_type), ]
        ...
        self.__rb_rwd = np.zeros((replay_buffer_size,) + environment.rwd_sp_shape, dtype=data_type)                               # Reward buffer
    ...
    def __sample_replay_buffer(self, replay_size):
        ...
        prf = np.random.random_sample((min(replay_size, self.__rb_entries),) + self.environment.prf_sp_shape)
        return self.__rb_obs[idx], self.__rb_act[idx], self.__rb_nobs[idx], self.__rb_rwd[idx], self.__rb_end[idx], prf
    ...
    def policy(self, obs):
        ...
        for w in self.__P[0:-2]:
            ...
            x = np.multiply(x, x > 0)
        # Compute the linear layer (Flattened Gaussian mean)
# OLD        x = np.tanh(np.add(np.matmul(x, self.__P[-2][0:-1, :]), self.__P[-2][-1:, :]))
    ...
    def __Pcompute(self, obs):
        ...
        :return: Numpy array representing the next action for each state
        '''
        a = obs.reshape((-1, self.environment.obs_sp_shape[-1]+self.environment.prf_sp_shape[-1]))
        # Compute the relu layers
        for w in self.__P[0:-2]:
            z = np.add(np.matmul(a, w[0:-1, :]), w[-1:, :])
            a = np.multiply(z, z > 0)
...
#OLD        u = np.tanh(z)
    ...
    def __Qcompute(self, obs, act):
        ...
        '''
        obs = obs.reshape((-1, self.environment.obs_sp_shape[-1]+self.environment.prf_sp_shape[-1]))
...
#                print("rand: ", np.random.normal(0.0, 1.0, 1))
                ...
                ## Forward pass
                a, k = batch_x.reshape((-1, self.environment.obs_sp_shape[-1]+self.environment.prf_sp_shape[-1])), []
                # Compute the relu layers
                for w in self.__P[0:-2]:
                    x = a
...
#OLD                dl_dz = np.multiply(dl_du, 1 - np.square(u))
        ...
        # Update alpha
        dl_da = np.mean(h) + self.entropy
        w = np.log2(self.__alpha)
        dl_dw = np.multiply(np.exp2(w), dl_da)
        self.__opt_adaptive_moment_gradient_descent(self.H_adam_alpha, self.H_adam_beta1, self.H_adam_beta2, w, dl_dw, self.__H_adam_aux)
        self.__alpha = np.exp2(w)
    ...
    def __Qtrain(self, x_ref, y_ref, epochs, n_batches=1):
        ...
        :return: Numpy array representing the loss in each training epoch
        '''
        x_ref = np.concatenate(x_ref, axis=-1).reshape((-1, self.environment.obs_sp_shape[0]+self.environment.prf_sp_shape[0]+self.environment.act_sp_shape[0]))
        n_batches = x_ref.shape[0] if n_batches > x_ref.shape[0] else n_batches
        loss_evolution = np.zeros((epochs, n_batches), dtype=data_type)
        batch_idxs = np.floor(np.linspace(0, x_ref.shape[0], n_batches+1)).astype(int)
...
#                    print("Q dense w:", w,"\ndl/dw:", dl_dw)
    ...
    def train(self, episodes, ep_steps, save_period=0, plot_period=0, tplot_period=5):
        ...
        ep_rwd = np.zeros((ep_steps, ) + env.rwd_sp_shape, dtype=data_type)      # Episode's rewards
        ...
        if self.__episode == 0:
            ...
            ep_ret[0:episode], ep_loss[0:episode] = self.__ep_ret[0:episode], self.__ep_loss[0:episode]
            self.__ep_ret, self.__ep_loss = ep_ret, ep_loss
        ...
        # For all episodes
        self.__plot(episode, env.get_pos(ep_obs[0:2]))
#        print("Q:", self.__Q)
#        print("P:", self.__P)
...
#            print("seed: ", self.seed)
            ...
            prf = np.random.random_sample(env.prf_sp_shape)

            # Do N steps following the policy estimator
            for s in range(ep_steps):
                # Decide the next action
                ep_act[s] = self.__Pcompute(np.concatenate((ep_obs[s], prf), axis=0))
            ...
            ep_rwd[0:s+1] = env.compute_rwd(ep_obs[0:s+1], ep_act[0:s+1], ep_obs[1:s+2], ep_rwd[0:s+1], prf)
            if not end: ep_rwd[s] += discount_factor * self.__Qcompute(np.concatenate((ep_obs[s+1], prf), axis=0), self.__Pcompute(np.concatenate((ep_obs[s+1], prf), axis=0)))
            for i in range(ep_len-2, -1, -1): ep_rwd[i] = ep_rwd[i] + discount_factor * ep_rwd[i + 1]
            ep_ret[episode, 0] = ep_rwd[0]
            ep_ret[episode, 1] = self.__Qcompute(np.concatenate((ep_obs[0], prf), axis=0), ep_act[0])
...
#            ep_ret[episode, :] = ep_ret[episode, :] / env.max_ret(ep_obs[0])
            ...
            for i in range(int(np.ceil(ep_len/Q_freq))):
                ...
                tr_obs, tr_act, tr_next_obs, tr_reward, tr_end, tr_prf = self.__sample_replay_buffer(replay_batch_size)
                tr_obs, tr_next_obs = env.compute_observed_state(tr_obs, tr_act, tr_next_obs, tr_prf)
                tr_reward = env.compute_rwd(tr_obs, tr_act, tr_next_obs, tr_reward, tr_prf)
                tr_obs, tr_next_obs = np.concatenate((tr_obs, tr_prf), axis=1), np.concatenate((tr_next_obs, tr_prf), axis=1)
...
#                    print("P:", self.__P)
            ...
            if (episode % plot_period) == 0: self.__plot(episode, env.get_pos(ep_obs[:ep_len+1]))
            elif (episode % tplot_period) == 0: self.__plot(episode, env.get_pos(ep_obs[:ep_len+1]), trajectory_only=True)
            print("Progress: ", episode, "/", episodes, "Alpha: ", self.__alpha, "Loss (Q,P):", ep_loss[episode-1, 0:2])

        # Update the episode number and save
...
#            print("rand save: ", np.random.normal(0.0, 1.0, 1))
    ...
    def test(self, ep_steps, preferences=None):
        '''
        ...
        if preferences is None:
            if (self.environment.prf_sp_shape[0] != 0):
                print("Error: Preferences are required for this environment.")
                return
            preferences = []
        ...
        preferences = np.array(preferences).reshape((1,)+env.prf_sp_shape)
        ...
        self.__plot(self.__episode, env.get_pos(ep_obs[0:2]))
        ...
        while True:
            ...
            # Act
            for s in range(ep_steps):
                obs = np.concatenate((ep_obs[s].reshape((1,) + env.obs_sp_shape), preferences), axis=1)
                act = self.policy(obs)                                          # Decide the next action
                ep_obs[s+1], _, end = env.act(act)                              # Act in the environment
                if end: break                                                   # End the episode on terminal state

            # Plot
            self.__plot(self.__episode, env.get_pos(ep_obs[:s+2]), trajectory_only=True)
    ...
    def __plot(self, episode, trajectory, trajectory_only=False):
        ...
        # Plot trajectory
        T_ax.set_title('Last Trajectory (N={0:d})'.format(trajectory.shape[0]-1))
        ...
        if (len(env.dst_pos) <= 1) or (len(trajectory) <= 1): return;
        T_ax.scatter(env.dst_pos[0], env.dst_pos[1], color='blue', alpha=0.8, linewidth=2, linestyle='solid')
        T_ax.scatter(env.int_pos[0], env.int_pos[1], color='lightblue', alpha=0.8, linewidth=2, linestyle='solid')
        T_ax.scatter(trajectory[0, 0], trajectory[0, 1], color='black', alpha=0.8, linewidth=2, linestyle='solid')
        T_ax.plot(trajectory[:, 0], trajectory[:, 1], color='black', alpha=0.3, linewidth=2, linestyle='solid')
        ...
        # Plot policy
#        obs = np.concatenate((np.zeros((len(self.__axis_2D),)+env.obs_sp_shape), np.ones((len(self.__axis_2D),)+env.prf_sp_shape)), axis=1)
#        obs[:, 0:2] = self.__axis_2D[:, 0:2]
        obs = np.concatenate((env.set_pos(self.__axis_2D), np.ones((self.__axis_2D.shape[0],)+env.prf_sp_shape)/env.prf_sp_shape[0]), axis=1)
        P_ax.invert_yaxis()
        ...
        act = np.zeros((self.__axis_2D.shape[0],)+env.act_sp_shape)
        Q_ax.invert_yaxis()
