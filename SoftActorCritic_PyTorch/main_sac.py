...
def SAC_Agent_Training(q):
    ...
    env = Environment(sim_measurement=31, obs_dim=17, act_dim=12, rwd_dim=7)
    ...
    # ---- The last 7 not seen by the agent but used in the reward (simulation only measurements)--------
    # position (x,y,z)
    ...
    pref_max_vector = np.array([2, 1, 1, 2, 1,0])
    pref_min_vector = np.array([0.5, 0, 0, 0, 0,0])
...
def updatePlot():
    global q, curve_Trajectory, curve_Trajectory_startPoint, curve_ForwardVelocity, curve_LateralVelocity, curve_ForwardAcc, curve_Pitch, \
        ...
        curve_Acc_rwd,curve_Energy_rwd, curve_P_Loss, curve_Q_Loss, curve_Real_Return, curve_Predicted_Return, curve_Return_Error, curve_Alpha, curve_Entropy, curve_Std, \
...
############################################### PLOTS #####################################################################
    ...
    curve_Energy_rwd = plot_Rewards.plot(pen=(255, 255, 255), name='Energy')
