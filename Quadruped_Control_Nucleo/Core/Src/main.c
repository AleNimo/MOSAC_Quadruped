...
#define K_TON (float32_t)((float32_t)140.0/(float32_t)13.0)
...
//PID Constants
#define KP (float32_t)0.1
#define KI (float32_t)0.01
  ...
  static uint8_t calibration_direction = 1;
	...
	switch(state)
	{
      ...
      if(cant_med == N_SAMPLES)
			...
			{
				for(uint8_t joint = 0; joint < JOINTS;joint++) joint_angle_acum[joint] += filt_angle_ADC[joint];
				time = 1000;
				cant_med++;
			}
			break;
