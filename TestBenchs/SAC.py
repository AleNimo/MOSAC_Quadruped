...
class SAC_Agent():
    def __init__(self, name, obs_dim, act_dim, rwd_dim, pref_max_vector, pref_min_vector, replay_buffer_size):
        ...
        self.replay_buffer = ReplayBuffer(replay_buffer_size, obs_dim, act_dim, rwd_dim, pref_max_vector, pref_min_vector)

        self.P_net = P_Network(obs_dim, act_dim, pref_dim, hidden1_dim=64, hidden2_dim=32,
                               alfa=0.0003, beta1=0.9, beta2=0.999)

        self.P_loss = torch.tensor(0, dtype=torch.float64).to(self.P_net.device)

        self.Q1_net = Q_Network(obs_dim, act_dim, pref_dim, hidden1_dim=128, hidden2_dim=64, hidden3_dim=32,
                                  alfa=0.0003, beta1=0.9, beta2=0.999, name='Q1_net')

        self.Q2_net = Q_Network(obs_dim, act_dim, pref_dim, hidden1_dim=128, hidden2_dim=64, hidden3_dim=32,
    ...
    def learn(self, step):
        if self.replay_buffer.mem_counter < self.replay_batch_size or self.replay_buffer.mem_counter < self.min_learn_steps:
            return
